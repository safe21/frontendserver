...
@Component({
  selector: 'app-editproject-ad',
  templateUrl: './editproject-ad.component.html',
  styleUrls: ['./editproject-ad.component.css']
})
export class EditprojectAdComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(async (params) => {
      ...
      this.studentId = await params['idadvisor'].substring(1);
  ...
  async getroleDescript(idproject: any, studentId: any) {
    try {
      const res: any = await this.http.get(this.api + "/roleDescript/:" + idproject + "/:" + studentId).toPromise();
